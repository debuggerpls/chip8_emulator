...
/* Set up an 3rdparty thread for using JNI with Android_JNI_GetEnv() */
...
int SDL_AndroidGetExternalStorageState(void)
{
    ...
    __android_log_print(ANDROID_LOG_INFO, "SDL", "3rdparty storage state: %s", state);
...
const char * SDL_AndroidGetExternalStoragePath(void)
{
    ...
    if (!s_AndroidExternalFilesPath) {
        ...
        if (!fileObject) {
            SDL_SetError("Couldn't get 3rdparty directory");
            LocalReferenceHolder_Cleanup(&refs);
            return NULL;
        }
